...
namespace Fralle.Core
{
  public static class Vector3Extensions
  {
    ...
    public static bool InViewPort(this Vector3 v) => v.x >= 0 && v.x <= 1 && v.y >= 0 && v.y <= 1 && v.z >= 0;
    ...
    public static Vector3 RoundedUp(this Vector3 t) => new Vector3 { x = Mathf.Round(t.x + .5f), y = Mathf.Round(t.y + .5f), z = Mathf.Round(t.z + .5f) };

    public static Vector3 RoundedDown(this Vector3 t) => new Vector3 { x = Mathf.Round(t.x - .5f), y = Mathf.Round(t.y - .5f), z = Mathf.Round(t.z - .5f) };

    public static Vector3 Rounded(this Vector3 t) => new Vector3 { x = Mathf.Round(t.x), y = Mathf.Round(t.y), z = Mathf.Round(t.z) };

    public static Vector3 Floored(this Vector3 t) => new Vector3 { x = Mathf.Floor(t.x), y = Mathf.Floor(t.y), z = Mathf.Floor(t.z) };
  }
}
